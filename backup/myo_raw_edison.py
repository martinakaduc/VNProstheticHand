...
class MLPerceptron(object):
    ...
    def __init__(self):
        for i in range(NUMS_OF_MOVES):
            with open('/root/VNProstheticHand/vals%d.dat' % i, 'ab') as f: pass
        self.read_data()

    def store_data(self, cls, vals):
        with open('/root/VNProstheticHand/vals%d.dat' % cls, 'ab') as f:
    ...
    def read_data(self):
        ...
        Y = []
        for i in range(NUMS_OF_MOVES):
            X.append(np.fromfile('/root/VNProstheticHand/vals%d.dat' % i, dtype=np.uint16).reshape(-1, 8))
    ...
    def train(self, X, Y):
        ...
        if HAVE_SK and self.X.shape[0] >= K * SUBSAMPLE:
            #self.clf = MLPClassifier(solver='adam', alpha=1e-5, hidden_layer_sizes=(8, ), batch_size = 'auto', random_state=1)
            self.clf = MLPClassifier(solver='lbfgs', hidden_layer_sizes=[100], max_iter=2000, activation='logistic')
            self.clf.fit(self.X[::SUBSAMPLE], self.Y[::SUBSAMPLE])
        else:
            self.clf = None
    ...
    def classify(self, d):
        ...
        d = np.reshape(d, (-1, NUMS_OF_SENSORS))
...
class RFClassifier(object):
    ...
    def train(self, X, Y):
        ...
        self.Y = Y
        if HAVE_SK and self.X.shape[0] >= K * SUBSAMPLE:
            self.rf = RandomForestClassifier(n_estimators=NUMS_OF_SENSORS+2)
            self.rf.fit(self.X[::SUBSAMPLE], self.Y[::SUBSAMPLE])
        else:
            self.rf = None
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            if attr == 0x27:
                ...
                ## something
                emg = vals[:NUMS_OF_SENSORS]
                moving = vals[NUMS_OF_SENSORS]
        ...
        for i, (u, v) in enumerate(zip(last_vals, vals)):
            if DRAW_LINES:
                pygame.draw.line(scr, (0,255,0),
                                 (w - D, int(h/NUMS_OF_SENSORS * (i+1 - u))),
                                 (w, int(h/NUMS_OF_SENSORS * (i+1 - v))))
                pygame.draw.line(scr, (255,255,255),
                                 (w - D, int(h/NUMS_OF_SENSORS * (i+1))),
                                 (w, int(h/NUMS_OF_SENSORS * (i+1))))
            else:
                c = int(255 * max(0, min(1, v)))
                scr.fill((c, c, c), (w - D, i * h / NUMS_OF_SENSORS, D, (i + 1) * h / NUMS_OF_SENSORS - i * h / NUMS_OF_SENSORS));
