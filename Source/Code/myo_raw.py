...
class RFClassifier(object):
    '''A wrapper for sklearn's random-forest classifier that stores
    training data in vals0, ..., vals15.dat.'''

    numOfY  = list(np.zeros(common.NUM_OF_MOVES))

    def __init__(self):
        self.read_data()

    def store_data(self, cls, vals):
        with open(common.EMG_DATA_DIR + 'vals%d.dat' % cls, 'ab') as f:
    ...
    def read_data(self):
        ...
        for i in range(common.NUM_OF_MOVES):
            if (not os.path.exists(common.EMG_DATA_DIR + 'vals%d.dat' %i)):
                X.append(np.array([], dtype=np.uint16).reshape(-1,8))
                Y.append(np.array([]))
                self.numOfY[i] = 0
            else:
                X.append(np.fromfile(common.EMG_DATA_DIR + 'vals%d.dat' % i, dtype=np.uint16).reshape(-1, 8))
                Y.append(i + np.zeros(X[-1].shape[0]))
                self.numOfY[i]  = X[-1].shape[0]

        if (os.path.exists(common.MODEL_DIR + common.MODEL_FILE)):
            self.X = np.vstack(X)
            self.Y = np.hstack(Y)
            with open(common.MODEL_DIR + common.MODEL_FILE, 'rb') as pickle_file:
                self.rf = pickle.load(pickle_file)
        else:
    ...
    def train(self, X, Y):
        self.X = X
        self.Y = Y
        if HAVE_SK and self.X.shape[0] >= common.K* common.SUBSAMPLES:
            self.rf = RandomForestClassifier(n_estimators=int(common.NUM_OF_SENSORS*1.5))
            self.rf.fit(self.X[::common.SUBSAMPLES], self.Y[::common.SUBSAMPLES])
        else:
            self.rf = None

    def saveModel(self, modelName):
        with open(common.MODEL_DIR + common.MODEL_FILE, 'wb') as pickle_file:
            pickle.dump(self.rf, pickle_file)
        print ('Model Saved!')
    ...
    def classify(self, d):
        if self.X.shape[0] < common.K* common.SUBSAMPLES: return 0
        if not HAVE_SK: return self.nearest(d)
        d = np.reshape(d, (-1, common.NUM_OF_SENSORS))
        return int(self.rf.predict(d)[0])

class MLPerceptron(object):
    """docstring for MLPerceptron."""
    numOfY  = [0] * common.NUM_OF_MOVES

    def __init__(self):
        for i in range(common.NUM_OF_MOVES):
            with open(common.EMG_DATA_DIR + 'vals%d.dat' % i, 'ab') as f: pass
        self.read_data()
    ...
    def read_data(self):
        ...
        Y = []
        for i in range(common.NUM_OF_MOVES):
            X.append(np.fromfile('emgData/vals%d.dat' % i, dtype=np.uint16).reshape(-1, 8))
            Y.append(i + np.zeros(X[-1].shape[0]))
            self.numOfY[i]  = X[-1].shape[0]
        self.train(np.vstack(X), np.hstack(Y))
    ...
    def train(self, X, Y):
        ...
        self.Y = Y
        if HAVE_SK and self.X.shape[0] >= common.K* common.SUBSAMPLES:
            self.clf = MLPClassifier(solver='lbfgs', hidden_layer_sizes=[100], max_iter=2000, activation='relu')
            self.clf.fit(self.X[::common.SUBSAMPLES], self.Y[::common.SUBSAMPLES])
        else:
            self.clf = None
    ...
    def classify(self, d):
        ...
        if not HAVE_SK: return self.nearest(d)
        d = np.reshape(d, (-1, common.NUM_OF_SENSORS))
        return int(self.clf.predict(d)[0])
...
class NNClassifier(object):
    ...
    def __init__(self):
        for i in range(common.NUM_OF_MOVES):
            with open('emgData/vals%d.dat' % i, 'ab') as f: pass
        self.read_data()

    def store_data(self, cls, vals):
        with open('emgData/vals%d.dat' % cls, 'ab') as f:
            f.write(pack('8H', *vals))
        self.train(np.vstack([self.X, vals]), np.hstack([self.Y, [cls]]))
        self.numOfY[cls] += 1
    ...
    def train(self, X, Y):
        ...
        if HAVE_SK and self.X.shape[0] >= common.K* common.SUBSAMPLES:
            ...
            self.nn.fit(self.X[::common.SUBSAMPLES], self.Y[::common.SUBSAMPLES])
        else:
            self.nn = None
...
class MyoRaw(object):
    ...
    def __init__(self, cls, tty=None):
        ...
        self.history = deque([0] * MyoRaw.HIST_LEN, MyoRaw.HIST_LEN)
    ...
    def emg_handler(self, emg, moving):
        ...
        if self.last_pose is None or (n > self.history_cnt[self.last_pose] + 5 and n > MyoRaw.HIST_LEN / 2):
            self.on_raw_pose(r)
            self.last_pose = r
    ...
    def connect(self):
        ...
        def handle_data(p):
            ...
            if attr == 0x27:
                ...
                ## something
                emg = vals[:common.NUM_OF_SENSORS]
                moving = vals[common.NUM_OF_SENSORS]
        ...
        for i, (u, v) in enumerate(zip(last_vals, vals)):
            if DRAW_LINES:
                pygame.draw.line(scr, (0,255,0),
                                 (w - D, int(h/common.NUM_OF_SENSORS * (i+1 - u))),
                                 (w, int(h/common.NUM_OF_SENSORS * (i+1 - v))))
                pygame.draw.line(scr, (255,255,255),
                                 (w - D, int(h/common.NUM_OF_SENSORS * (i+1))),
                                 (w, int(h/common.NUM_OF_SENSORS * (i+1))))
            else:
                c = int(255 * max(0, min(1, v)))
                scr.fill((c, c, c), (w - D, i * h / common.NUM_OF_SENSORS, D, (i + 1) * h / common.NUM_OF_SENSORS - i * h / common.NUM_OF_SENSORS));

        pygame.display.flip()
        last_vals = vals

    m = MyoRaw(RFClassifier(), sys.argv[1] if len(sys.argv) >= 2 else None)
